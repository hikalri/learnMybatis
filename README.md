...
- [MyBatis 学习项目](#mybatis-学习项目)
  - [项目结构](#项目结构)
  - [1. MyBatis 配置](#1-mybatis-配置)
    - [1.1 类型别名 (Type Aliases)](#11-类型别名-type-aliases)
    - [1.2 环境配置 (Environments)](#12-环境配置-environments)
    - [1.3 映射器 (Mappers)](#13-映射器-mappers)
  - [2. MyBatis生命周期和作用域](#2-mybatis生命周期和作用域)
    - [2.1 SqlSessionFactory](#21-sqlsessionfactory)
    - [2.2 SqlSession](#22-sqlsession)
    - [2.3 Mapper](#23-mapper)
  - [3. resultMap结果映射](#3-resultmap结果映射)
    - [3.1 基本结果映射](#31-基本结果映射)
      - [3.1.1 resultMap的使用方式](#311-resultmap的使用方式)
      - [3.1.2 resultMap的优势](#312-resultmap的优势)
      - [3.1.3 resultMap与resultType的区别](#313-resultmap与resulttype的区别)
      - [3.1.4 自动映射与resultMap结合使用](#314-自动映射与resultmap结合使用)
    - [3.2 继承结果映射](#32-继承结果映射)
  - [4. 日志工厂](#4-日志工厂)
    - [4.1 MyBatis日志适配机制](#41-mybatis日志适配机制)
      - [4.1.1 日志接口适配](#411-日志接口适配)
      - [4.1.2 手动指定日志实现](#412-手动指定日志实现)
    - [4.2 项目日志配置](#42-项目日志配置)
      - [4.2.1 MyBatis的特殊处理](#421-mybatis的特殊处理)
      - [4.2.2 日志布局模式](#422-日志布局模式)
    - [1. 逐段拆解分析](#1-逐段拆解分析)
    - [2. 重点细节：为什么要写 `%-5level`？](#2-重点细节为什么要写--5level)
    - [3. 重点细节：`%logger{36}` 的缩写策略](#3-重点细节logger36-的缩写策略)
    - [4. 组合后的完整效果](#4-组合后的完整效果)
    - [5. 进阶玩法：彩色日志（Coloring）](#5-进阶玩法彩色日志coloring)
    - [4.3 Java代码中手动输出日志](#43-java代码中手动输出日志)
      - [4.3.1 基本日志使用](#431-基本日志使用)
    - [4.4 日志最佳实践](#44-日志最佳实践)
  - [5. 分页](#5-分页)
    - [5.1 分页参数](#51-分页参数)
    - [5.2 分页查询](#52-分页查询)
    - [5.3 分页结果](#53-分页结果)
  - [6. Lombok](#6-lombok)
  - [7. 多对一、一对多处理](#7-多对一一对多处理)
    - [7.1 多对一关系](#71-多对一关系)
      - [7.1.1 关系模型](#711-关系模型)
      - [7.1.2 MyBatis实现](#712-mybatis实现)
      - [7.1.3 MyBatis合并机制](#713-mybatis合并机制)
    - [7.2 一对多关系](#72-一对多关系)
      - [7.2.1 关系模型](#721-关系模型)
      - [7.2.2 MyBatis实现](#722-mybatis实现)
      - [7.2.3 MyBatis合并机制](#723-mybatis合并机制)
    - [7.3 延迟加载](#73-延迟加载)
      - [7.3.1 MyBatis延迟加载配置](#731-mybatis延迟加载配置)
      - [7.3.2 延迟加载实现](#732-延迟加载实现)
      - [7.3.3 延迟加载触发机制](#733-延迟加载触发机制)
      - [7.3.4 延迟加载的优势](#734-延迟加载的优势)
      - [7.3.5 注意事项](#735-注意事项)
  - [8. 动态SQL](#8-动态sql)
    - [8.1 为什么需要动态SQL](#81-为什么需要动态sql)
      - [8.1.1 不使用动态SQL的问题示例](#811-不使用动态sql的问题示例)
      - [8.1.2 使用动态SQL的优势](#812-使用动态sql的优势)
    - [8.2 if条件判断](#82-if条件判断)
    - [8.3 set标签](#83-set标签)
    - [8.4 其他动态SQL标签](#84-其他动态sql标签)
  - [9. 缓存](#9-缓存)
    - [9.1 一级缓存](#91-一级缓存)
      - [9.1.1 作用域范围](#911-作用域范围)
      - [9.1.2 缓存失效条件](#912-缓存失效条件)
      - [9.1.3 缓存存储结构](#913-缓存存储结构)
      - [9.1.4 缓存命中机制](#914-缓存命中机制)
      - [9.1.5 缓存大小限制](#915-缓存大小限制)
      - [9.1.6 一级缓存的优势与注意事项](#916-一级缓存的优势与注意事项)
    - [9.2 二级缓存](#92-二级缓存)
      - [9.2.1 作用域范围](#921-作用域范围)
      - [9.2.2 缓存生效条件](#922-缓存生效条件)
      - [9.2.3 缓存更新机制](#923-缓存更新机制)
      - [9.2.4 缓存存储结构](#924-缓存存储结构)
      - [9.2.5 缓存同步机制](#925-缓存同步机制)
      - [9.2.6 缓存配置参数](#926-缓存配置参数)
      - [9.2.7 二级缓存的优势与注意事项](#927-二级缓存的优势与注意事项)
    - [9.3 第三方缓存Ehcache配置](#93-第三方缓存ehcache配置)
    - [9.4 缓存写入时机](#94-缓存写入时机)
      - [9.4.1 一级缓存写入时机](#941-一级缓存写入时机)
      - [9.4.2 二级缓存写入时机](#942-二级缓存写入时机)
      - [9.4.3 缓存写入的配置控制](#943-缓存写入的配置控制)
    - [9.5 查询缓存顺序](#95-查询缓存顺序)
      - [9.5.1 MyBatis缓存查询流程](#951-mybatis缓存查询流程)
      - [9.5.2 缓存查询的性能优化](#952-缓存查询的性能优化)
  - [运行项目](#运行项目)
  - [总结](#总结)
